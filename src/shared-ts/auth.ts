...
export function authRequired(
	inOneOfGroups: string[] | undefined = undefined,
): (req: AuthHttpRequest) => Promise<HttpResponse | undefined> {
	const JWKS = jose.createRemoteJWKSet(
		new URL(
			"https://cognito-idp.eu-west-1.amazonaws.com/eu-west-1_Fzpf4i9XY/.well-known/jwks.json",
		),
	);
	...
	return async (req: AuthHttpRequest) => {
		...
		try {
			...
			const { payload, protectedHeader } = await jose.jwtVerify(
				...
				{
					issuer:
						"https://cognito-idp.eu-west-1.amazonaws.com/eu-west-1_Fzpf4i9XY",
